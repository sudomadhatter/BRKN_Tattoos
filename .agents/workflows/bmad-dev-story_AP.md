...
- **Never** `git commit`/`push`, and **never** set the story status (the orchestrator flips it to
  `review` after its own green gate; the human owns `review -> done`) — stay out of status.
