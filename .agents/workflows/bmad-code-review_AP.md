...
`@.agents/rules/bmad_code_review_fast_path.md` applies — run it yourself, sequentially, no subagents,
...
- Never `git commit`/`push`; never set the story status or edit `sprint-status.yaml`. The ORCHESTRATOR
  flips the story to `review` after its own green test gate (never `done` — the human owns `review -> done`);
  you, the agent, stay out of status entirely.
